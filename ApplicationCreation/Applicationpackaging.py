...
sys.path.append(
    f"{os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), os.pardir))}\\Modules\\"
)
    ...
    with open(f"{file_path}\\{file_name}", "w") as f:
        ...
        for element in file_text:
            if element == "from Core import mainDownloader":
                ...
                pos = (
                    pos - 2
                )  # This will give above value, where second blank line resides
                f.seek(pos)
            else:
                f.write(element + "\n")
...
subprocess.call(
    f"""python -m PyInstaller --clean {file_path}\\Downloader.spec""",
    cwd=application_folder_dir,
)
...
copyfile(
    f"{os.path.dirname(file_path)}\\README.md", f"{application_folder_dir}\\README.md"
)
